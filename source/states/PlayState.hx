...
class PlayState extends MusicBeatState {
	...
	function applyRecolorToCharacter(char:Character) {
		...
		if (map != null && map.keys().hasNext()) {
			backend.RecolorUtil.recolor(char, map);
		}
	}
	...
	override public function create() {
		...
		if (!stageData.hide_girlfriend) {
			...
			applyRecolorToCharacter(gf);
		}

		dad = new Character(0, 0, SONG.player2);
		startCharacterPos(dad, true);
		dadGroup.add(dad);
		applyRecolorToCharacter(dad);

		boyfriend = new Character(0, 0, SONG.player1, true);
		startCharacterPos(boyfriend);
		boyfriendGroup.add(boyfriend);
		applyRecolorToCharacter(boyfriend);
...
backend.RecolorUtil.recolor(healthBar, bfData);
